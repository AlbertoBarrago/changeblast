...
`serval <path>` (root command, no subcommand) is a convenience alias for
`serval inspect <path>`. See the `serval --help` output (generated from
...
dependency, per `serval doctor`) rather than parsing `.git` internals or
...
`.serval.yml`'s `historyWindow.days`/`historyWindow.maxCommits`
...
unless a repository overrides it via `.serval.yml`'s
...
`serval diff [<ref>]` (`cmd/diff.go`) computes `git diff --name-only <ref>`
...
tree, then runs the same `inspectTarget` pipeline used by `serval inspect`
...
would be unusable across dozens or hundreds of files. `serval diff`'s
...
committed for `man serval` to work after install. `make man-check`
...
`.goreleaser.yml` builds `serval` for darwin/linux/windows ×
...
a model-serving daemon. Serval never manages credentials for any
...
`serval doctor` probes `$OLLAMA_HOST` (or `http://localhost:11434`) with
